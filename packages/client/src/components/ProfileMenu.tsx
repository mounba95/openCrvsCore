...
const ProfileMenuComponent = ({
  ...
  // Niger : accès direct à la bascule de commune depuis le menu profil
  // (visible sur la page d'accueil et toutes les autres pages), en plus de
  // Paramètres — voir ChangeOfficeModal.tsx.
  const {
    canSwitchOffice,
    officeName,
    showModal,
  ...
  const getMenuItems = (intl: IntlShape): IToggleMenuItem[] => {
    ...
    if (canSwitchOffice) {
      items.push({
        icon: <Icon name="MapPin" size="small" />,
        label: intl.formatMessage(userSetupMessages.changeOfficeMenuItem),
        handler: openModal
      })
    }
    items.push({
  ...
  const getMenuHeader = (userDetails: User | null): JSX.Element => {
    ...
    return (
      <>
        ...
        {officeName && <UserOffice>{officeName}</UserOffice>}
      </>
    )
  }
