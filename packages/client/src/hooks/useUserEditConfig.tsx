...
export function useUserEditConfig(
  ...
  // Niger : même référence de juridiction que le champ "user.office"
  // ci-dessous, exposée pour le sélecteur multi-commune (OfficesPicker) qui
  // est rendu en dehors de ce pipeline déclaratif — voir UserEditor.tsx.
  const officeFieldAllowedLocations = user.jurisdiction(
    user.scope(isNewUser ? 'user.create' : 'user.edit').attribute('accessLevel')
  )

  return {
    officeFieldAllowedLocations,
    getConfig: () =>
      ({
        id: '__user__',
        analytics: false,
