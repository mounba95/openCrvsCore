...
export const useHomePage = () => {
  ...
  // Niger : commune ACTIVE (bascule sans reconnexion) si définie, sinon la
  // commune permanente — lecture directe du store (pas de useCurrentUser
  // ici, pour ne pas rendre ce hook dépendant de Suspense : il est utilisé
  // par des composants qui n'en sont pas entourés, ex. HistoryNavigator).
  const activeOfficeId = useActiveOfficeStore((s) => s.activeOfficeId)
  const currentOfficeId = activeOfficeId ?? userDetails?.primaryOfficeId
