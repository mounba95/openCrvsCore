...
import { ITokenPayload, User, UUID } from '@opencrvs/commons/client'
import { useActiveOfficeStore } from '@client/v2-events/hooks/useActiveOffice'
import { trpcClient } from '@client/v2-events/trpc'

/**
 * Niger : libère tous les actes assignés à l'utilisateur avant de couper sa
 * session — mêmes raisons que `Sidebar.tsx`'s `logout()`, mais ce chemin
 * (menu profil "Déconnexion", expiration de session, etc.) passe par
 * REDIRECT_TO_AUTHENTICATION plutôt que par cette fonction. Best-effort :
 * un échec ne doit jamais bloquer la déconnexion elle-même.
 */
async function unassignAllMine() {
  try {
...
> = (
  ...
  switch (action.type) {
    ...
    case actions.CHECK_AUTH: {
      ...
      // Niger : commune choisie à l'écran "StepThree" du login (bascule
      // initiale), à appliquer avant la toute première requête tRPC — doit
      // être lue avant le history.replaceState ci-dessous, qui vide l'URL.
      const activeOfficeIdFromUrl = new URLSearchParams(
        window.location.search
      ).get('activeOfficeId')
      if (activeOfficeIdFromUrl) {
        useActiveOfficeStore
          .getState()
          .setActiveOffice(activeOfficeIdFromUrl as UUID)
      } else if (refreshTokenFromUrl) {
        // Niger : un handoff explicite (nouveau login) peut concerner un tout
        // autre utilisateur que celui de la session précédente — la commune
        // "active" persistée en localStorage ne doit jamais lui survivre. Un
        // nouvel utilisateur non affecté à cette commune verrait sinon TOUTES
        // ses requêtes rejetées en FORBIDDEN par resolveUserDetails (events),
        // sans jamais atteindre l'application (page qui tourne indéfiniment).
