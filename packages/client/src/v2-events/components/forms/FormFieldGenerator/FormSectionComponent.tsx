...
/**
 * Groups a flat list of page fields into visual sections, using PAGE_HEADER
 * fields as section-boundary markers. Fields before the first PAGE_HEADER
 * (or on pages that don't use one at all) stay ungrouped, preserving the
 * default OpenCRVS look. This is purely a rendering-time grouping — it does
 * not touch field ids/data paths, unlike FieldType.FIELD_GROUP.
 */
function chunkFieldsByHeader(
  fields: FieldConfig[]
): { header: FieldConfig | null; fields: FieldConfig[] }[] {
  const chunks: { header: FieldConfig | null; fields: FieldConfig[] }[] = []
  let current: { header: FieldConfig | null; fields: FieldConfig[] } = {
    header: null,
    fields: []
  }

  for (const field of fields) {
    if (field.type === FieldType.PAGE_HEADER) {
      if (current.header !== null || current.fields.length > 0) {
        chunks.push(current)
      }
...
export function FormSectionComponent({
  ...
  return (
    <section className={className}>
      {chunkFieldsByHeader(pageFields).map((chunk, chunkIdx) => {
        const renderedFields = chunk.fields.map(renderField)

        if (!chunk.header) {
          // eslint-disable-next-line react/no-array-index-key
          return <React.Fragment key={chunkIdx}>{renderedFields}</React.Fragment>
        }

        if (!isFieldVisible(chunk.header, ocrvsFullForm, validatorContext)) {
          return null
        }
        ...
        return (
          <InputField
            ...
            hideErrorLabel
            id={chunk.header.id}
            label={intl.formatMessage(chunk.header.label)}
            touched={false}
            variant="highlighted"
          >
            {renderedFields}
