...
export function useUsers() {
  ...
  return {
    ...
    // Niger : contexte de l'utilisateur connecté, reflétant sa commune ACTIVE
    // (bascule sans reconnexion) — distinct de getUser, qui renvoie toujours
    // la commune réelle/permanente de l'utilisateur consulté, quel qu'il soit.
    getMyContext: {
      // Niger : ne PAS retirer queryFn ici (contrairement à getUser/getUsers
      // ci-dessus) — celui-ci n'a pas de queryFn par défaut enregistré via
      // setQueryDefaults, donc le retirer laisse React Query sans aucune
      // fonction pour exécuter la requête ("Missing queryFn").
      useSuspenseQuery: () => {
        return useSuspenseQuery({
          ...trpc.user.getMyContext.queryOptions(),
          queryKey: trpc.user.getMyContext.queryKey()
        }).data
      },
      // Niger : variante non suspensive — pour les appelants qui ne sont pas
      // garantis d'être rendus sous une limite <Suspense> (ex :
      // useCertifierSelectorFieldConfig.ts, appelé directement dans le corps
      // de la page d'impression, sans limite Suspense ajoutée autour).
      useQuery: () => {
        return useQuery({
          ...trpc.user.getMyContext.queryOptions(),
          queryKey: trpc.user.getMyContext.queryKey()
    ...
    // Niger : communes affectées à un utilisateur CONSULTÉ (pas forcément
    // l'utilisateur connecté) — sert à pré-remplir le sélecteur multi-commune.
    getOfficeIds: {
      useQuery: (id: string, options?: { enabled?: boolean }) => {
        return useQuery({
          ...trpc.user.getOfficeIds.queryOptions(id),
          ...options,
          queryKey: trpc.user.getOfficeIds.queryKey(id)
        })
      }
    },
    createUser: ({
