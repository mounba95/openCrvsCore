...
export const usePrintableCertificate = ({
  ...
  certificateConfig,
  language,
  // Niger : sur la page de confirmation d'impression, l'OEC signataire a
  // déjà été choisi (étape précédente du même flux) — contrairement à
  // l'onglet Dossier (simple consultation, pas d'action d'impression en
  // cours), l'aperçu à l'écran doit donc déjà montrer le bloc "Certifiée
  // conforme" avec cet OEC, pas seulement le PDF final après confirmation.
  // Voir Review.tsx (passe false) vs SoucheDocumentPreview.tsx (garde true).
  review = true
}: {
  event: EventDocument
  config: EventConfig
