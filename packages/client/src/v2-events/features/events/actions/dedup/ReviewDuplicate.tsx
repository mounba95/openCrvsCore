...
function ReviewDuplicate() {
  ...
  // Niger : les actes candidats "doublon potentiel" ne sont pas forcément
  // déjà présents dans le cache local (ex: un Admin qui vient de déclarer
  // et est immédiatement redirigé ici n'a jamais ouvert l'autre acte) —
  // auparavant, `findLocalEventDocument` renvoyait `undefined` et la ligne
  // ci-dessous levait une exception qui faisait planter toute la page
  // ("Event with id ... not found in cache."). On télécharge maintenant les
  // candidats manquants en arrière-plan et on réaffiche une fois prêts,
  // au lieu de planter.
  const [missingCandidatesFetchVersion, setMissingCandidatesFetchVersion] =
    useState(0)

  useEffect(() => {
    const missingIds = eventState.potentialDuplicates
      .map(({ id }) => id)
      .filter((id) => !findLocalEventDocument(id))
    ...
    void Promise.all(
      missingIds.map((id) =>
        trpcClient.event.get.query({ eventId: id, waitFor: false })
      )
    ).then((fetchedEvents) => {
      if (cancelled) {
        return
      ...
      setMissingCandidatesFetchVersion((version) => version + 1)
    })
  ...
  }, [eventState.potentialDuplicates, missingCandidatesFetchVersion])
  ...
  >((acc, { id, trackingId }) => {
    ...
    // Niger : téléchargement encore en cours (voir useEffect ci-dessus) —
    // on affiche les onglets disponibles, celui-ci apparaîtra dès que le
    // fetch en arrière-plan aura abouti et déclenché un nouveau rendu.
    if (!localEvent) {
      return acc
    }
    acc[trackingId] = {
  ...
  return (
    ...
    >
      ...
      ) : !potentialDuplicates[selectedTab] ? (
        // Niger : le téléchargement en arrière-plan de cet acte candidat
        // (voir useEffect plus haut) n'est pas encore terminé.
        <SuspenseLoadingFallback id="duplicate-candidate-loading" />
      ) : (
        <DuplicateComparison
          originalEvent={event}
