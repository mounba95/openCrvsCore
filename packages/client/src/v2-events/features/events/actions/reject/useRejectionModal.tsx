...
export function useRejectionModal(eventId: UUID, eventType: string) {
  ...
  async function handleRejection(onClose: () => void) {
    ...
    if (modalResult) {
      // Niger : on attend la confirmation serveur (et l'invalidation du
      // cache qui l'accompagne) avant de fermer — sinon la file d'où l'on
      // vient (ex. celle de l'OEC) affiche encore l'acte, qui semble ne
      // jamais être passé côté "acte à corriger".
      await events.actions.reject.mutateAsync({
        eventId,
        declaration: {},
        transactionId: uuid(),
