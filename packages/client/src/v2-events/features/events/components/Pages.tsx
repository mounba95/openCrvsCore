...
import { ContentSize } from '@opencrvs/components/lib/Content'
import {
  FormFieldGenerator,
...
interface PagesProps {
  ...
  /** Niger : voir FormWizard.tsx — passé tel quel. */
...
export function Pages({
  ...
  // Tracks the furthest page the user has reached in this session, so the
  // step list can let them jump freely between any already-visited page
  // (not just one step back at a time) without losing entered data.
  const [highestVisitedIdx, setHighestVisitedIdx] = useState(
    Math.max(pageIdx, 0)
  )
  useEffect(() => {
    if (pageIdx > highestVisitedIdx) {
      setHighestVisitedIdx(pageIdx)
    }
  }, [pageIdx, highestVisitedIdx])
  ...
  function onJumpToPage(targetPageId: string) {
    onPageChange(targetPageId)
  }

  // Niger : "Aller à la révision" n'est utile qu'une fois toutes les pages
  // remplies — on ne le propose donc que sur la toute dernière page, pas
  // comme raccourci depuis n'importe quelle étape intermédiaire.
  const isLastPage = pageIdx === visiblePages.length - 1

  const wizardProps = {
    currentPage: pageIdx,
    pageTitle: intl.formatMessage(page.title),
    showReviewButton: !hideBackToReview && isLastPage,
    onNextPage,
    onPreviousPage,
