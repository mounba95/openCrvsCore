...
import { SoucheDocumentPreview } from '@client/v2-events/features/events/components/SoucheDocumentPreview'
...
function ReadonlyViewContent({ eventId }: { eventId: UUID }) {
  ...
  useEffect(() => {
    // Niger : le cache "view-event" est en staleTime: Infinity et persiste
    // dans IndexedDB — il survit même à une déconnexion/reconnexion. Si un
    // AUTRE utilisateur modifie l'acte pendant ce temps (ex: enregistrement,
    // correction), cet onglet Dossier continue d'afficher l'ancienne version
    // indéfiniment. Un rafraîchissement en arrière-plan à l'ouverture
    // corrige ça sans changer le comportement partagé (refetchOnMount reste
    // false pour tous les autres usages de ce cache).
    if (isOnline) {
      void queryClient.refetchQueries({ queryKey: [['view-event', eventId]] })
    }
  ...
  const soucheTemplate = useSoucheCertificateTemplate(
    event.type,
  ...
  // Niger : quand un gabarit "Volet 1 / Souche" existe pour ce type d'acte,
  // l'onglet "Dossier" affiche directement ce document (aperçu à l'écran du
  // même gabarit utilisé à l'impression), façon INCI — plutôt que le
  // formulaire en accordéon ci-dessous, utilisé en repli si aucun gabarit
  // Souche n'est encore configuré pour ce type d'acte.
  if (soucheTemplate) {
    return (
      <SoucheDocumentPreview
