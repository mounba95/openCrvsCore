...
export async function deleteLocalEvent(updatedEvent: EventDocument) {
  ...
  // Niger : volontairement PAS attendu — `handleDeclaration` (menu d'action)
  // attend la résolution complète de cette fonction avant de naviguer vers
  // la file suivante. Pour un compte avec beaucoup de files déjà visitées en
  // cache (ex: Admin, jusqu'à 13), rafraîchir chacune en parallèle avant de
  // laisser la navigation se poursuivre pouvait prendre plusieurs secondes,
  // perçues comme "l'écran se rafraîchit plusieurs fois avant de s'arrêter".
  // Ce rafraîchissement se termine désormais en arrière-plan ; la file de
  // destination récupère de toute façon des données fraîches à son
  // montage. `refetchDraftsList` ci-dessous reste attendu : c'est le
  // correctif du bug "reste dans Brouillon", qui doit rester garanti avant
  // de considérer l'action terminée.
  void refetchAllSearchQueries()
  return refetchDraftsList()
}
