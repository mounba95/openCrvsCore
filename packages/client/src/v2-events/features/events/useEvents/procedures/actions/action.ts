...
/**
 * Niger : bug trouvé le 2026-08-11 — cette fonction ne renvoyait JAMAIS
 * `false` pour une erreur non-409 (ex. une exception JS ordinaire, sans
 * `.data.httpStatus`) : elle affichait le toast générique une fois au
 * 10ᵉ échec, mais continuait à relancer indéfiniment ensuite, en silence
 * (aucune erreur console, react-query n'affiche pas les tentatives
 * intermédiaires) — c'est ce qui rendait certaines actions (notamment les
 * corrections bloquées) invisiblement "en boucle" pendant plusieurs
 * minutes sans aucun retour à l'écran. Corrigé : on arrête vraiment après
 * 10 tentatives, comme sur un conflit 409.
 */
function retryUnlessConflict(
  ...
  // Niger, 2026-08-22 : un 403 (scope manquant du token JWT — ex. juste
  // après l'ajout d'un nouveau scope dans roles.ts, avant reconnexion de
  // l'utilisateur) ne peut JAMAIS réussir en réessayant : le token ne change
  // pas entre deux tentatives. Sans cet arrêt immédiat, on retombait dans le
  // même clignotement déjà diagnostiqué une première fois pour
  // markNotDuplicate (10 tentatives avec backoff exponentiel, ~60-90s).
...
/**
 * Niger : délai avant nouvelle tentative après un échec transitoire (pas un
 * conflit 409, voir `retryUnlessConflict`) — ex. clic sur "Imprimer", qui
 * tente `assignment.assign` avant de naviguer (voir `useEventActionsOnClick.
 * tsx`) et bloque l'affichage (icône qui tourne) tant que la tentative n'a
 * pas abouti. L'ancien délai (`Math.max(10000, ...)`) attendait AU MOINS 10
 * secondes avant même la première nouvelle tentative, quelle que soit la
 * cause de l'échec — un simple aléa réseau ponctuel se traduisait par 10 à
 * 30+ secondes d'icône qui tourne avant l'ouverture de la page d'impression
 * (signalé par l'utilisateur le 2026-08-09). Départ rapide (500ms), doublé
 * à chaque tentative, plafonné à 15s pour ne pas marteler un serveur
 * réellement indisponible.
 */
function retryDelay(attemptIndex: number) {
  return Math.min(15000, 500 * 2 ** attemptIndex)
}

const lockedByMessage = {
  id: 'event.conflict.lockedBy',
  defaultMessage: 'This record is locked by {name}, {officeName}',
  description:
    'Toast shown when trying to act on a record currently assigned to someone else'
}
...
const forbiddenMessage = {
  id: 'errors.forbiddenScope',
  defaultMessage:
    "Vous n'avez pas la permission d'effectuer cette action. Déconnectez-vous puis reconnectez-vous.",
  description: 'Toast shown when a scope is missing from the current session'
}

/**
 * Niger : le premier utilisateur à cliquer sur une action (déclarer/valider/
 * enregistrer/imprimer...) prend l'acte ; un autre utilisateur qui clique
 * ensuite sur la même action se voit refuser la prise (409) — c'est à CE
 * moment-là que ce message doit apparaître (voir CONTEXTE-PROJET.md,
 * `useEventActionsOnClick.tsx` tente `assignment.assign` avant de naviguer
 * vers l'action elle-même). Quand le serveur peut nommer qui détient l'acte
 * (`error.data.lockedBy`, résolu par `requireAssignment`), on l'affiche
 * nommément — c'est le SEUL message à afficher dans ce cas (le générique
 * "vous avez été désassigné", repli précédent, a été retiré : consulter un
 * acte verrouillé par quelqu'un d'autre doit rester silencieux, seule une
 * tentative d'action dessus doit prévenir, nommément).
 *
 * Niger, 2026-08-22 : réintroduit un repli générique pour le cas SANS
 * `lockedBy` (l'acte n'est détenu par personne, juste plus par nous) — sans
 * lui, une soumission qui échouait à ce moment précis (ex. Déclarer, qui
 * requiert désormais une ré-assignation juste avant l'envoi) ne montrait
 * strictement rien à l'écran, laissant l'utilisateur devant un bouton qui
...
function errorToastOnConflict(error: TRPCClientError<AppRouter>) {
  ...
  const lockedBy = (
    error.data as { lockedBy?: { id: string; name: string; officeName?: string } }
  ).lockedBy
  if (lockedBy) {
    showToast({
      message: lockedByMessage,
      toastType: 'error',
      toastId: `locked-by-${lockedBy.id}`,
      messageOpts: { name: lockedBy.name, officeName: lockedBy.officeName ?? '' }
    })
  } else {
    showToast({
      message: unassignedMessage,
      toastType: 'error',
      toastId: 'unassigned-conflict'
    })
  }
}
...
const customMutationKeys = {
  ...
  declareAndValidate: [['declareAndValidate']],
...
interface CustomMutationTypes {
  ...
  declareAndValidate: customApi.CustomMutationParams
...
queryClient.setMutationDefaults(customMutationKeys.declareAndValidate, {
  mutationFn: waitUntilEventIsCreated(customApi.declareAndValidate),
  retry: retryUnlessConflict,
  retryDelay,
  onSuccess: deleteLocalEventAndToastOnDuplicate,
  onError: errorToastOnConflict,
...
export function useEventCustomAction<T extends CustomMutationKeys>(
  ...
  const buildMutationInput = (
    params: Omit<CustomMutationTypes[T], 'eventConfiguration'>
  ) => {
  ...
  return {
    mutate: (params: Omit<CustomMutationTypes[T], 'eventConfiguration'>) =>
      mutation.mutate(buildMutationInput(params)),
    // Niger : nécessaire pour pouvoir attendre la fin réelle de l'action
    // avant de fermer/naviguer (voir DeclareActionMenu.tsx) — sans ça, la
    // navigation se faisait immédiatement après un simple appel fire-and-
    // forget, ce qui pouvait laisser l'interface dans un état incohérent si
    // la requête échouait après coup.
    mutateAsync: (params: Omit<CustomMutationTypes[T], 'eventConfiguration'>) =>
      mutation.mutateAsync(buildMutationInput(params))
  }
}
