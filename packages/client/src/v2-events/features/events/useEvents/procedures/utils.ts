  ...
  return waitUntilEventIsCreated<inferInput<P>, inferOutput<P>>(
    async ({ eventType, ...params }) => {
      ...
      if (params.type === ActionType.ASSIGN) {
        // Niger, 2026-08-22 : préchargement en arrière-plan uniquement — ne
        // JAMAIS attendre sa fin avant de renvoyer la réponse de l'ASSIGN.
        // Ce préchargement (cache fichiers/utilisateurs pour chaque doublon
        // potentiel) peut prendre plusieurs secondes quand plusieurs
        // doublons sont détectés, ce qui bloquait inutilement toute action
        // qui enchaîne sur un ASSIGN (ex. la navigation vers la page de
        // comparaison de doublons après Déclarer) alors que cette page sait
        // déjà charger ses candidats manquants elle-même en arrière-plan.
        void prefetchPotentialDuplicates(params.eventId)
      }
      return response
    }
  )
