...
export function useEvents() {
  ...
  return {
    ...
    actions: {
      ...
      assignment: {
        assign: {
          ...
          // Niger : contrairement à `mutate` (tir-et-oublie), cette version
          // renvoie une vraie promesse qui échoue si l'acte est détenu par
          // quelqu'un d'autre — utilisée par `useEventActionsOnClick` pour
          // tenter de prendre l'acte AU MOMENT du clic sur une action, et
          // n'enchaîner sur l'action réelle (déclarer/valider/imprimer...)
          // que si la prise a réussi.
          mutateAsync: async ({
            eventId,
            assignedTo,
            refetchEvent
    ...
    customActions: {
      ...
      declareAndValidate: useEventCustomAction('declareAndValidate'),
