...
function toCertificateVariables(
  ...
  /**
   * Niger : les formations sanitaires ont un nom abrégé (`name`, utilisé
   * dans la liste déroulante de recherche) et un nom complet officiel
   * (`fullName`) affiché ici — au récapitulatif et sur l'acte imprimé.
   * `fullName` est absent pour les autres types de Location (ex: bureaux
   * CRVS), qui retombent donc sur `name` comme avant.
   */
  const displayName =
    (location as Location | undefined)?.fullName || location?.name || ''
