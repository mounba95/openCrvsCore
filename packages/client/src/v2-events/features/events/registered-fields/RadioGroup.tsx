...
function RadioGroupInput({
  ...
  const resolvedSize =
    configuration?.styles?.size === 'NORMAL' ? RadioSize.NORMAL : RadioSize.LARGE

  return (
    <RadioGroupComponent
      {...props}
      data-testid={props.id}
      // Niger : les groupes de radios compacts ("NORMAL", ex. Sexe) sont
      // affichés côte à côte plutôt qu'empilés, façon CityWeb.
      flexDirection={resolvedSize === RadioSize.NORMAL ? 'row' : undefined}
      name={props.id}
