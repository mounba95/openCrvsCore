...
import { Button, Icon, Link, Pagination } from '@opencrvs/components'
...
const messages = defineMessages({
  ...
  backToRecord: {
    ...
    description:
      'Label for the button that returns from the history page to the record visualisation',
    id: 'events.overview.history.backToRecord'
  }
})

/**
 * Niger : bouton retour vers la visualisation de l'acte (Dossier) — l'audit
 * n'étant plus un onglet mais un lien direct depuis le menu Action, il lui
 * faut son propre moyen de revenir en arrière.
 */
function BackToRecordButton({ eventId }: { eventId: UUID }) {
  const intl = useIntl()
  const navigate = useNavigate()
  const [{ backTo }] = useTypedSearchParams(ROUTES.V2.EVENTS.EVENT.AUDIT)
  ...
  return (
    ...
    >
      ...
      {intl.formatMessage(messages.backToRecord)}
    </Button>
  )
}
...
function EventHistorySkeleton({ eventId }: { eventId: UUID }) {
  const intl = useIntl()
  return (
    <Content
      ...
      topActionButtons={[
        <BackToRecordButton key="back-to-record" eventId={eventId} />
      ]}
    >
      <LargeGreyedInfo />
    </Content>
...
function EventHistory({ fullEvent }: { fullEvent: EventDocument }) {
  ...
  return (
    <Content
      ...
      title={intl.formatMessage(messages.audit)}
      topActionButtons={[
        <BackToRecordButton key="back-to-record" eventId={fullEvent.id} />
      ]}
    >
      <TableDiv>
        <Table
...
export function EventHistoryIndex() {
  ...
  if (!shouldShowHistory) {
    return <EventHistorySkeleton eventId={eventId} />
  }
