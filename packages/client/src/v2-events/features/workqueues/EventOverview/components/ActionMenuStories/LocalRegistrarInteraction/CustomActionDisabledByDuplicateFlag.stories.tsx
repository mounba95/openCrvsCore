...
export const ApproveActionStateTransitions: StoryObj = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      'Auto-assign on open: Confirm is still disabled (flag still present)',
      async () => {
        // Niger : l'acte s'assigne automatiquement et silencieusement à
        // l'ouverture (plus de clic manuel "Assign"/AssignModal) — on attend
        // juste que la mutation déclenchée au montage se termine, en
        // observant "Unassign" apparaître dans le menu.
        await userEvent.click(
          await canvas.findByRole('button', { name: 'Action' })
        )
