...
function prepareComments(history: ActionDocument) {
  ...
  // Niger : raison fournie par country-config pour un enregistrement
  // rejeté (ex: registre non ouvert) — voir RegisterAction.reason.
  if (history.type === ActionType.REGISTER && history.reason) {
    comments.push({ comment: history.reason })
  }
