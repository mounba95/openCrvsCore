  ...
  const customActionConfigs = useMemo(() => {
    return eventConfiguration.actions
      ...
      .map((action) => ({
        ...
        // Niger : plus de pré-condition d'assignation ici — onCustomAction
        // prend l'acte au clic (voir useEnsureAssignedToSelf.ts), comme les
        // actions standard.
        disabled: !isActionEnabled(action, event, validatorContext),
