...
export function useResolveAssignmentActionConditionals(event: EventIndex) {
  ...
  const resolveConditionals = useCallback(
    (actionType: typeof ActionType.UNASSIGN) => {
      const { enabled, visible } = resolveActionConditionals({
        event,
        actionType,
    ...
    [event, validatorContext, isActionAllowedForUser, eventConfiguration, isOnline]
  )
...
export function useAssignmentActionConfigurationResolver(event: EventIndex) {
  ...
  const { onUnassign, modal } = useAssignmentActions(event)

  const resolveAction = useCallback(
    (actionType: typeof ActionType.UNASSIGN): ActionMenuItem => {
      ...
      return {
        ...
        icon: isValidIcon(actionConfig?.icon)
          ? actionConfig.icon
          : actionIcons[actionType],
        onClick: async () => onUnassign(),
        disabled: !enabled,
        hidden: !visible
      }
    },
    [resolveConditionals, eventConfiguration, onUnassign]
  )
