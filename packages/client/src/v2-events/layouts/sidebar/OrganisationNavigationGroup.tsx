    ...
    <>
      {hasAccess(TAB_GROUPS.organisations) && (
        <NavigationGroup>
          ...
          {hasAccess(WORKQUEUE_TABS.registers) && (
            <NavigationItem
              icon={() => <Icon name="FileText" size="small" />}
              id={`navigation_${WORKQUEUE_TABS.registers}`}
              isSelected={currentWorkqueueSlug === WORKQUEUE_TABS.registers}
              label={intl.formatMessage(
                navigationMessages[WORKQUEUE_TABS.registers]
              )}
              onClick={() => {
          ...
          {hasAccess(WORKQUEUE_TABS.declarationDeadlines) && (
            <NavigationItem
              icon={() => <Icon name="Clock" size="small" />}
              id={`navigation_${WORKQUEUE_TABS.declarationDeadlines}`}
              isSelected={
                currentWorkqueueSlug === WORKQUEUE_TABS.declarationDeadlines
              }
              label={intl.formatMessage(
                navigationMessages[WORKQUEUE_TABS.declarationDeadlines]
              )}
              onClick={() => {
                navigate(routes.DECLARATION_DEADLINES)
                menuCollapse?.()
              }}
            />
          )}
