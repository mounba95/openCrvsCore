...
export const UserSummary = User.pick({
  ...
  /**
   * Niger : affiché en permanence dans la liste des utilisateurs — il n'y a
   * plus de rappel d'identifiant par e-mail/SMS, l'admin doit pouvoir le
   * consulter à tout moment. Absent pour un System (voir SystemSummary).
   */
 ...
 * chiffres accepté) — plus simple pour des agents qui définissent
 * eux-mêmes des mots de passe courts.
 */
export const UserPassword = z.string().min(6)
...
export const CreateUserInput = User.pick({
  ...
  .extend({
    ...
    /**
     * Niger : l'administrateur définit le mot de passe directement à la
     * création (avec confirmation côté client) — plus de mot de passe
     * temporaire envoyé par e-mail/SMS ni d'écran d'activation séparé. Voir
     * CONTEXTE-PROJET.md.
     */
    password: UserPassword,
    signature: FileFieldValue.optional(),
    /**
     * Niger : communes affectées à l'utilisateur, en plus de sa commune
     * principale (primaryOfficeId). Si absent, seule la commune principale
     * est affectée — comportement inchangé par rapport à avant.
     */
    officeIds: z.array(UUID).optional()
  })
