...
/**
 * Niger : formes dupliquées volontairement (pas importées depuis
 * `./events/EventIndex`/`EventMetadata`/`Flag`) — `scopes.ts` et
 * `authentication.ts` s'importent déjà mutuellement, et tout le sous-arbre
 * `events/*` finit par redépendre d'`authentication.ts` (via
 * `ActionDocument.ts`), donc un import direct y crée une vraie dépendance
 * circulaire (confirmé : `ExactStatus`/`AnyOfStatus` valaient `undefined`
 * au moment de l'évaluation de `z.union([...])` ci-dessous, faisant
 * planter tout le module au chargement). Les valeurs de statut sont
 * stables au niveau du noyau (pas configurables par pays), donc cette
 * duplication ne devrait jamais diverger ; `flags` reste volontairement
 * en `z.string()` simple plutôt que le schéma `Flag` complet (qui
 * importerait la même chaîne problématique) — la validation stricte des
 * indicateurs se fait déjà ailleurs (config des événements), pas ici.
 */
const ScopeEventStatus = z.enum([
  'CREATED',
  'NOTIFIED',
  'DECLARED',
  'REGISTERED',
...
// Ensure input is always an array for consistent parsing, even if a single
// string is provided by qs (a 1-element array round-trips through
// qs.stringify/qs.parse as a bare scalar, not `[x]` — same reasoning as
// `scopeByEvent`/`userRole` above).
const StringArrayFromScalarOrArray = z.preprocess(
  (val) => (val === undefined ? undefined : [val].flat()),
  z.array(z.string())
)
const ExactStatus = z.object({
  type: z.literal('exact'),
  term: ScopeEventStatus
})
const AnyOfStatus = z.object({
  type: z.literal('anyOf'),
  terms: z.preprocess(
    (val) => (val === undefined ? undefined : [val].flat()),
    z.array(ScopeEventStatus)
  )
})
const ContainsFlags = z.object({
  anyOf: StringArrayFromScalarOrArray.optional(),
  noneOf: StringArrayFromScalarOrArray.optional(),
  allOf: StringArrayFromScalarOrArray.optional()
})
...
const AllRecordScopeOptions = scopeOptionsDeclaredOrNotified
  .extend({
    ...
    /**
     * Niger : restreint le scope au statut courant de l'acte (ex: `record.read`
     * de l'OEC limité à `anyOf: [CREATED, NOTIFIED, DECLARED]`, jamais
     * REGISTERED — voir CONTEXTE-PROJET.md pour le cycle Agent de
     * Saisie/Agent Vérificateur/OEC). Absent = aucune restriction (identique
     * au comportement historique).
     */
    status: z.union([ExactStatus, AnyOfStatus]).optional(),
    /**
     * Niger : restreint le scope aux indicateurs (`flags`) courants de
     * l'acte (ex: `record.read` de l'Agent Vérificateur limité à
     * `{noneOf: ['validated']}` — il perd l'accès une fois l'acte validé,
     * et le retrouve après REGISTER puisque cette action retire toujours
     * l'indicateur `validated`). Même forme que le DSL de requête des
     * workqueues (`ContainsFlags`). Absent = aucune restriction.
     */
...
const ResolvedScopeOptionsFull = ResolvedScopeOptionsDeclared.extend({
  ...
  // Niger : passent tels quels — ne dépendent pas du user/lieu, donc rien à résoudre.
  status: z.union([ExactStatus, AnyOfStatus]).optional(),
  flags: ContainsFlags.optional()
}).describe(
...
export const Scope = z.discriminatedUnion('type', [
  ...
  z.object({
    // Niger : gestion des registres d'état civil (création/clôture
    // annuelle par commune) — voir opencrvs-countryconfig/src/api/registers.
    type: z.literal('organisation.manage-registers'),
    options: AccessLevelOptions.optional()
  }),
  z.object({
