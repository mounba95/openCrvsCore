...
const event = Object.assign(eventFn, {
  ...
  /**
   * Niger : référence un champ de déclaration pour une colonne de file de
   * travail — soit le même id pour tous les types d'événement, soit une
   * map id-par-type-d'événement (le cas courant pour une file mélangeant
   * plusieurs actes). Voir WorkqueueColumnConfig.ts.
   */
  declaration(
    fieldIdOrMap: string | Record<string, string>
  ): WorkqueueColumnDeclarationValue {
    return {
      $declaration: fieldIdOrMap
    }
  },
  /**
   * Niger : libellé statique par type d'événement pour une colonne de file
   * de travail (ex. "Rôle") — pas une donnée de déclaration.
   */
  roleLabel(
    labelsByEventType: Record<string, TranslationConfig>
  ): WorkqueueColumnRoleLabelValue {
    return {
      $roleLabel: labelsByEventType
    }
  }
})
