...
/**
 * Plage numérique pour un champ stocké en `keyword` (chaîne) mais dont la
 * valeur est un nombre — ex. `registrationNumber`, attribué de façon
 * séquentielle SANS zéros de remplissage ("1", "2", ... "957"). Un `Range`
 * classique ferait une comparaison lexicographique incorrecte sur ce type de
 * champ ("100" < "20"), d'où ce type dédié qui est traduit, côté indexation,
 * en requête `script` (cast en nombre) plutôt qu'en `range` natif ES.
 * `gte`/`lte` sont tous deux optionnels pour permettre une plage ouverte
 * (uniquement "à partir de" ou uniquement "jusqu'à").
 */
...
export const QueryExpression = z
  .object({
    ...
    'legalStatuses.REGISTERED.registrationNumber': z.optional(
      z.union([Exact, NumericRange])
    ),
