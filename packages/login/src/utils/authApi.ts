...
export interface IMyContextResponse {
...
/**
 * Niger : liste des communes affectées à l'utilisateur qui vient de
 * s'authentifier — sert à décider si l'étape "choix de la commune active"
 * doit s'afficher avant la redirection vers l'application (voir StepThree).
 * Exposée en REST simple (pas via tRPC) car ce micro-frontend n'a pas de
 * client tRPC — voir `getMyContext` dans opencrvs-core/packages/events.
 */
const getMyContext = (token: string): Promise<IMyContextResponse> => {
  return request({
    url: '/api/events/my-context',
    method: 'GET',
