...
export type VerifyCodeAction = {
  ...
  payload: { code: string; inAppRedirect: () => void }
}

export type VerifyCodeCompleteAction = {
  type: typeof VERIFY_CODE_COMPLETED
  payload: ITokenResponse & { inAppRedirect: () => void }
}
...
export const authenticate = (
  ...
  /**
   * used when the server still requires a 2FA code (TWO_FA_ENABLED=true)
   */
  toStepTwo: () => void,
  /**
   * used when the server already returned a token (2FA disabled) and the
   * user has more than one commune assigned
   */
  toStepThree: () => void
  ...
  return {
    type: AUTHENTICATE,
    payload: { ...cleanedData, toStepTwo, toStepThree }
  }
}
...
export const completeAuthentication = (
  ...
  payload: { ...response, toStepTwo, toStepThree }
})
...
export const verifyCode = (
  values: IVerifyCodeNumbers,
  inAppRedirect: () => void
): VerifyCodeAction => {
  const code = Object.values(values).join('')
  return {
    type: VERIFY_CODE,
    payload: { code, inAppRedirect }
  }
}
