    ...
    configureServer(server: import('vite').ViteDevServer) {
      server.middlewares.use((req, res, next) => {
        if (req.url?.startsWith('/register')) {
          ...
          // Use the incoming request's own hostname (not a hardcoded
          // 'localhost') so this redirect also works when the app is
          // accessed over the LAN, e.g. from a phone during PWA testing.
          const hostname = (req.headers.host ?? 'localhost:3020').split(
            ':'
          )[0]
          res.writeHead(302, {
            Location: `http://${hostname}:3000${suffix}`
          })
          res.end()
          return
        }
