  ...
  switch (input.type) {
    case ActionType.REGISTER: {
      return {
        ...
        ...(input.reason ? { content: { reason: input.reason } } : {})
      }
    }
...
export async function addAction(
  ...
  if (input.type !== ActionType.READ && input.type !== ActionType.ASSIGN) {
    ...
    /**
     * Niger : trouvé le 2026-08-11 — un simple échec de ce nettoyage
     * (fichiers non référencés en trop dans le stockage documentaire, ex.
     * jeton expiré au moment de l'appel — cas fréquent sur les formulaires
     * longs à plusieurs pages, comme celui de correction) faisait planter
     * toute la fonction alors que l'action elle-même (créée juste
     * au-dessus) avait déjà réussi. Comme cet appel se situe hors du
     * try/catch de `defaultRequestHandler`, l'action restait alors
     * orpheline en statut "Requested" pour toujours, sans jamais atteindre
     * sa confirmation. Le nettoyage de fichiers est une tâche d'hygiène
     * secondaire — son échec ne doit jamais pouvoir bloquer l'action
     * principale déjà enregistrée avec succès.
     */
    try {
      await cleanupUnreferencedFiles(updatedEvent, token)
    } catch (error) {
      logger.error(
        `Failed to clean up unreferenced files for event ${input.eventId}`,
        error
      )
    }
  }
