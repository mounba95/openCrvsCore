...
function toEventDocument(
  ...
  const notNullActions = actions.map(({ actionType, content, ...action }) =>
    dropNulls({
      ...action,
      type: actionType,
      content,
      // REGISTER stores its rejection reason inside `content` (there is no
      // dedicated `reason` column), but the RegisterAction document schema
      // still expects it as a bare `reason` field — promote it back here.
      ...(actionType === ActionType.REGISTER &&
      content &&
      typeof content === 'object' &&
      'reason' in content
        ? { reason: (content as Record<string, unknown>).reason }
        : {})
    })
  )
