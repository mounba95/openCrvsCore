...
/**
 * Niger : remplace l'ensemble des communes affectées à un utilisateur.
 * `officeIds` doit toujours inclure la commune principale (primaryOfficeId).
 */
export async function setUserOfficesInTrx(
  trx: Kysely<Schema>,
  userId: UUID,
  officeIds: UUID[]
) {
  await trx.deleteFrom('userOffices').where('userId', '=', userId).execute()
  await trx
    .insertInto('userOffices')
    .values(officeIds.map((officeId) => ({ userId, officeId })))
    .execute()
}

export async function getUserOfficeIds(userId: UUID): Promise<UUID[]> {
  const db = getClient()
  const rows = await db
    .selectFrom('userOffices')
    .select('officeId')
    .where('userId', '=', userId)
    .execute()
...
export async function isUserAssignedToOffice(
  userId: UUID,
  officeId: UUID
): Promise<boolean> {
  const db = getClient()
