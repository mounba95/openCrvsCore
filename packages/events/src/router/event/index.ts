...
export const eventRouter = router({
  ...
  actions: router({
    ...
    assignment: router({
      ...
      // Niger : appelée à la déconnexion (voir client) — libère tous les
      // actes actuellement assignés à l'utilisateur, pour qu'aucun autre
      // agent ne reste bloqué par un verrou oublié après un logout. C'est
      // toujours une auto-désassignation (assignedTo === user.id), donc
      // aucun scope particulier n'est requis (voir unassignRecord).
      unassignAllMine: userOnlyProcedure
        .output(z.object({ unassignedCount: z.number() }))
        .mutation(async ({ ctx }) => {
          const { user, token } = ctx
          const eventConfigs = await getInMemoryEventConfigurations(token)

          const { results } = await findRecordsByQuery({
            search: {
              query: {
                type: 'and',
                clauses: [
          ...
          for (const event of results) {
            ...
            const result = await unassignRecord({
              input: {
          ...
          return { unassignedCount: results.length }
        })
    }),
    correction: router({
      request: router(
