...
interface ActionProcedureConfig {
  ...
  // Niger : schéma optionnel pour parser le corps d'une réponse HTTP 400
  // (rejet) de l'API de confirmation country-config — par défaut, ce corps
  // est ignoré (voir `defaultRequestHandler`). Utilisé par REGISTER pour
  // faire remonter un `reason` explicite (ex: registre non ouvert).
  actionRejectionResponseSchema?: z.ZodObject<z.ZodRawShape>
  meta?: OpenApiMeta
}

const defaultConfig = {
  actionConfirmationResponseSchema: undefined,
  actionRejectionResponseSchema: undefined
} as const
...
const ACTION_PROCEDURE_CONFIG = {
  ...
  [ActionType.REGISTER]: {
    ...
    actionRejectionResponseSchema: z.object({
...
export async function defaultRequestHandler(
  ...
  /**
   * Niger : si quoi que ce soit échoue de façon inattendue entre l'envoi de
   * la demande et sa résolution (échange de jeton, appel réseau à
   * country-config, réponse illisible...), l'action précédemment créée en
   * statut 'Requested' (ci-dessus) ne doit JAMAIS rester orpheline — sinon
   * l'acte reste bloqué indéfiniment (aucune action possible, "désassigné"
   * à la moindre nouvelle tentative), sans qu'aucun message clair n'explique
   * pourquoi. On la fait donc basculer explicitement en 'Rejected' avant de
   * remonter l'erreur au client (même mécanisme que le rejet asynchrone
   * normal — voir `reject` procedure plus bas), pour que l'acte reste
   * toujours dans un état récupérable.
   */
  async function rejectOrphanedPendingAction() {
    // Niger : defaultRequestHandler ne sert que les actions passant par le
    // flux de confirmation notify/country-config (voir
    // getDefaultActionProcedures) — input.type est donc toujours l'un des
    // types acceptés par addAsyncRejectAction, malgré le typage plus large
    // hérité d'ActionInputWithType.
    await addAsyncRejectAction(
      {
        transactionId: input.transactionId,
        originalActionId: requestedAction.id,
        type: input.type as Parameters<typeof addAsyncRejectAction>[0]['type'],
        keepAssignment: false,
        waitFor: input.waitFor
      },
      { user, event: eventWithRequestedAction, configuration }
    )
  }
  ...
  try {
    ...
    // If we get an unexpected failure response, reject the pending action so
    // the record doesn't stay stuck, then surface the error to the client.
    if (responseStatus === ActionConfirmationResponse.UnexpectedFailure) {
      await rejectOrphanedPendingAction()
      throw new TRPCError({
    ...
    // Niger : n'importe quelle autre exception (ex: l'échange de jeton
    // `getActionConfirmationToken` échoue) doit elle aussi débloquer l'acte
    // au lieu de le laisser en 'Requested' indéfiniment.
    logger.error(error)
    await rejectOrphanedPendingAction()
    throw new TRPCError({
      code: 'INTERNAL_SERVER_ERROR',
      message: 'Unexpected failure while confirming the action'
    })
  }
}
    ...
    request: userTypeBasedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const result = await defaultRequestHandler(
          ...
          actionConfig.actionRejectionResponseSchema
        )
