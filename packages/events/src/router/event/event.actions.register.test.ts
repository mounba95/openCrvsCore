...
describe('Request and confirmation flow', () => {
  ...
  describe('Synchronous confirmation flow', () => {
    ...
    test('should not save action if notify API returns invalid registration number', async () => {
      ...
      // Niger : un payload invalide ne doit plus laisser l'acte bloqué
      // indéfiniment en 'Requested' — voir rejectOrphanedPendingAction dans
    ...
    test(`should save ${ActionStatus.Rejected} action if notify API returns HTTP 500, instead of leaving it stuck as ${ActionStatus.Requested}`, async () => {
      const { user, generator } = await setupTestCase()
      const client = createTestClient(user, [
        encodeScope({ type: 'record.read' }),
        encodeScope({ type: 'record.create' }),
      ...
      // Niger : un échec inattendu de l'API de confirmation ne doit plus
      // laisser l'acte bloqué indéfiniment en 'Requested' — voir
      // rejectOrphanedPendingAction dans defaultRequestHandler.
      expect(registerActions).toEqual([
        expect.objectContaining({
          status: ActionStatus.Requested
        }),
