...
> = async ({ input, next, ctx }) => {
  ...
  if (user.type === TokenUserType.enum.user && user.id !== assignedTo) {
    // Niger : on résout qui détient l'acte (nom + bureau) pour que le client
    // puisse afficher "Verrouillé par {nom}, {bureau}" au lieu d'un message
    // générique — l'assignation elle-même reste inchangée, seul ce message
    // est enrichi (voir aussi `resolveLockedBy`, réutilisé par `assignRecord`
    // pour le même besoin lors d'une tentative de reprise d'un acte).
    const lockedBy = assignedTo
      ? await resolveLockedBy(assignedTo)
      : undefined
