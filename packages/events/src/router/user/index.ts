...
export const userRouter = router({
  ...
  /**
   * Niger : liste des communes affectées à un utilisateur consulté (pas
   * forcément l'utilisateur connecté) — sert à pré-remplir le sélecteur
   * multi-commune lors de l'édition d'un utilisateur. Même sensibilité que
   * `get`, donc même garde d'accès.
   */
  getOfficeIds: userOnlyProcedure
    .input(UUID)
    .use(userCanReadOtherUser)
    .output(z.array(UUID))
    .query(async ({ input }) => getUserOfficeIds(input)),
  /**
   * Niger : contexte de l'utilisateur CONNECTÉ tel que résolu pour cette
   * requête — reflète la commune active (bascule via X-Active-Office-Id),
   * contrairement à `get`, qui renvoie toujours la commune réelle/permanente
   * de l'utilisateur consulté, quel qu'il soit. Sert aussi à lister les
   * communes affectées, pour le sélecteur de bascule.
   *
   * Également exposée en REST simple (via trpc-to-openapi, `protect: true` —
   * toujours authentifiée) : `packages/login` (micro-frontend séparé, sans
   * client tRPC) l'appelle juste après le 2FA pour savoir si un écran de
   * choix de commune doit s'afficher avant de rediriger vers l'application.
   */
  getMyContext: userOnlyProcedure
    .meta({
      openapi: {
        summary: "Contexte de l'utilisateur connecté (commune active incluse)",
        description:
          'Renvoie primaryOfficeId/administrativeAreaId tels que résolus pour cette requête (bascule de commune active comprise), ainsi que la liste des communes affectées.',
        method: 'GET',
        path: '/my-context',
        tags: ['User'],
        protect: true
    ...
    .output(
      UserContext.extend({
        officeIds: z.array(UUID),
        // Niger : noms des communes affectées — utilisé par packages/login
        // (écran de choix de commune), qui n'a pas accès aux données de
        // localisation mises en cache côté client.
        offices: z.array(z.object({ id: UUID, name: z.string() }))
      })
    )
    .query(async ({ ctx }) => {
      const officeIds = await getUserOfficeIds(UUID.parse(ctx.user.id))
      const locations = await getLocations({ locationIds: officeIds })
      const offices = officeIds.map((id) => ({
        id,
        name: locations.find((location) => location.id === id)?.name ?? id
      }))
      return { ...ctx.user, officeIds, offices }
    }),
  create: userAndSystemProcedure
    .input(CreateUserInput)
    .use(canCreateUserWithScopes(['user.create']))
    // Niger : l'identifiant généré doit être communiqué à l'admin (plus de
    // notification par e-mail/SMS) — on l'inclut donc dans la réponse, en plus
    // du schéma User habituel qui ne l'expose jamais.
    .output(User.extend({ username: z.string() }))
  ...
  sendResetPasswordInvite: userAndSystemProcedure
    ...
    .input(z.object({ userId: UUID, password: PasswordSchema }))
    .mutation(async ({ input, ctx }) => {
      const auditLogIdentifiers = getAuditLogIdentifiers(ctx.token)
      await resetPasswordByAdmin(input.userId, input.password)
      await writeAuditLog({
