...
// Niger : le mot de passe est désormais fourni directement par l'appelant
// (voir CreateUserInput) — plus de mot de passe temporaire généré/envoyé.
const TEST_PASSWORD = 'ValidTestPassw0rd1'
...
test('Allows user creation when with the right scope', async () => {
  ...
  expect(createdUser).toMatchObject({
    ...
    role: userPayload.role,
    // Niger : l'admin définit le mot de passe directement à la création
    // (voir CreateUserInput.password) — le compte est donc actif
    // immédiatement, plus de statut 'pending' en attente d'activation.
    status: 'active'
  })
...
/**
 * Niger : le mot de passe est désormais fourni directement par l'admin
 * (avec confirmation côté client), il n'y a plus de mot de passe
 * temporaire généré/envoyé — un mot de passe est désormais requis, avec
 * seulement une longueur minimale de 6 caractères (voir UserPassword dans
 * @opencrvs/commons).
 */
test('Rejects user creation when password is missing or too weak', async () => {
  const { user } = await setupTestCase()
