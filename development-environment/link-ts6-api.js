...
const CONSUMERS = [
  ...
  // Niger : le seul `ts-node` du repo vit ici, imbriqué sous
  // cosmiconfig-typescript-loader (voir node_modules/.bin/ts-node) — c'est
  // lui qui exécute directement les services back-end en dev (nodemon
  // `ts-node -r tsconfig-paths/register src/index.ts` pour auth, events...),
  // et son propre bootstrap CLI (ts-node/dist/bin.js) fait require('typescript')
  // au démarrage, plantant pareil sur TS7 sans ce lien.
  'cosmiconfig-typescript-loader/node_modules/ts-node'
]

// Niger : ts-node résout aussi le compilateur en remontant depuis le RÉPERTOIRE
// DU PROJET exécuté (`packages/<pkg>`), pas seulement depuis son propre dossier
// — donc le lien ci-dessus (dans node_modules/) ne suffit pas pour les services
// lancés directement via `nodemon --exec ts-node ... src/index.ts` (auth,
// documents, gateway, events, data-seeder, migration). Chacun a aussi besoin
// de son propre lien local.
const TS_NODE_PACKAGE_CONSUMERS = [
  'auth',
  'documents',
  'gateway',
  'events',
...
for (const pkg of TS_NODE_PACKAGE_CONSUMERS) {
  const pkgDir = path.join(ROOT, 'packages', pkg)
  if (!fs.existsSync(pkgDir)) {
    console.warn(`link-ts6-api: packages/${pkg} not found, skipping`)
    continue
  }
  ...
  fs.rmSync(link, { recursive: true, force: true })
  fs.symlinkSync(path.relative(nested, TS6), link, 'dir')
  console.log(`link-ts6-api: packages/${pkg} → typescript@6 (API)`)
}
